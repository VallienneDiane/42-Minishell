...
/*   Updated: 2022/04/05 16:16:49 by dvallien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    cmd->fd_out_trunc = open(cmd->tab_redir_out1[0], O_CREAT | O_RDWR | O_TRUNC, 0644);
    if (!cmd->tab_redir_out1 || cmd->fd_out_trunc == -1)
    {
        printf("minishell: OUT1 %s : No such file or directory\n", cmd->tab_redir_out1[0]);
        exit(EXIT_FAILURE);
    }
    return (0);
}
...
{
    cmd->fd_out_append = open(cmd->tab_redir_out2[0], O_CREAT | O_RDWR | O_APPEND, 0644);
