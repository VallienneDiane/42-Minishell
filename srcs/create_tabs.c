...
/*   Updated: 2022/04/06 17:31:40 by dvallien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    ft_malloc_tab(t_cmd *cmd)
{
    ...
    cmd->tab_redir_in = ft_calloc(sizeof(char *), (cmd->nb_in + cmd->nb_heredoc + 1));
...
void    ft_create_tab(t_cmd *cmd, t_list *list, int current_block)
{
    ...
    while (list && list->block == current_block)
    {
        ...
        else if (ft_strncmp("redir_in", list->type, 8) == 0 || ft_strncmp("heredoc", list->type, 7) == 0)
        {
